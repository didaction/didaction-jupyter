...
- **Controls:** The header’s 44px outlined sidebar-icon button toggles the workspace, with a directional chevron, action tooltip, accessible name, and expanded state. It remains visible when the sidebar is hidden. The sidebar shows a “Notebooks” heading, a wrapping workspace-relative folder path, and explicit Up and Refresh actions; Up is disabled at the workspace root.
